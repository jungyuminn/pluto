...
<p align="center">
  할 일, 일정, 일기, 가계부를 한곳에서 보는 Flutter 앱입니다.<br>
  안드로이드·iOS·웹(<a href="https://pluto.day">pluto.day</a>)을 같은 코드로 만듭니다.
</p>

<p align="center">
  <img alt="Flutter" src="https://img.shields.io/badge/Flutter-3-02569B?logo=flutter&logoColor=white">
  <img alt="version" src="https://img.shields.io/badge/version-1.4.0-5B8DEF">
  <img alt="platform" src="https://img.shields.io/badge/Android%20%7C%20iOS%20%7C%20Web-5B8DEF">
</p>
...
취준은 할 일과 면접 일정이 섞이고, 기업마다 전형이 다릅니다. 플루토는 달력 위에 그걸 올려 두고, 홈에서 오늘·내일·남은 일을 바로 보게 하려는 앱입니다. 취업이 아니어도 일기·가계부·언젠가와 같이 쓸 수 있습니다.

로그인하지 않으면 데이터는 **이 기기**에만 남습니다. 카카오·구글·애플로 로그인하면 클라우드로 맞추고, 친구의 일정만 미니캘린더로 볼 수 있습니다. zip·드라이브·아이클라우드 백업으로 옮기거나 복구할 수도 있습니다.

## 무엇을 하나요

| 화면 | 하는 일 |
| --- | --- |
| **홈** | 오늘·내일 할 일, 남은 할 일, 언젠가. 로그인하면 위쪽에 나와 친구가 보이고, 카드를 길게 눌러 순서를 바꿀 수 있습니다. |
| **캘린더** | 할 일·기업 전형을 한 달에 모읍니다. 기간·반복·여러 날을 고를 수 있고, 날짜마다 일기·가계부를 남길 수 있습니다. |
| **기업** | 지원 중인 회사를 카드로 두고, 전형과 카테고리로 나눠 봅니다. |
| **설정** | 테마·글꼴·알림·로그인·백업·캘린더 가져오기·앱 둘러보기 |

### 친구

로그인한 사람만 쓸 수 있습니다. 아이디로 요청을 보내고, 상대가 수락해야 친구가 됩니다.

- 홈에서 상대를 누르면 **미니캘린더**가 열리고, 그 사람의 **일정 라벨**만 보입니다.
- 일기·가계부·메모는 열리지 않습니다.
- 친구 목록에서 끊으면 더 이상 보이지 않습니다.

### 로그인과 클라우드

- 카카오, 구글, 애플로 로그인할 수 있습니다. 웹은 [pluto.day](https://pluto.day)입니다.
- 로그인하면 할 일·일기·파일이 계정 클라우드에 맞춰집니다.
- 설정에서 카테고리 자동 고르기를 켜면, 처음 보는 제목은 로그인한 뒤 AI가 내 칸 중에서 고릅니다.
...
- 로그인 전에는 할 일·기업·일기를 기기의 SharedPreferences와 앱 문서 폴더에 둡니다.
- 설정에서 zip으로 백업하고, 최근 3개까지 앱과 다운로드에 남겨 둡니다. 안드로이드는 구글 드라이브, 아이폰은 아이클라우드에도 둘 수 있습니다.
...
ios/          iOS
web/          웹 (pluto.day)
functions/    로그인·친구 Cloud Functions
