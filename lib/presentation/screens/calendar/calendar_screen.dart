...
class _CalendarScreenState extends State<CalendarScreen>
  ...
  Future<void> _goToCurrentMonth() async {
    ...
    final now = DateTime.now();
    final current = DateTime(now.year, now.month);
    if (_visibleMonth.year == current.year &&
        _visibleMonth.month == current.month) {
      return;
    }
    final delta =
        (current.year - _baseMonth.year) * 12 +
        (current.month - _baseMonth.month);
    final target = _initialPage + delta;
    final distance = (target - _pages.page!.round()).abs();
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              CalendarMonthHeader(
                month: _visibleMonth,
                onTitlePressed: _goToCurrentMonth,
